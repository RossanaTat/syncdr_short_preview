...
# ET Consultant Application - (DECID)

## Coding sample

The code provided below is extracted from an R package -{syncdr}- I developed to facilitate file handling, directory comparison, and synchronization.
Please note that this document offers a brief overview of the {syncdr} R package, highlighting some of its key functionalities for directory synchronization. {syncdr} includes a wide range of tools designed to manage file synchronization efficiently. Since the code in this sample is not runnable on its own, this document also includes an "Output" section, which serves as a practical example to demonstrate the functions in action without requiring you to run the code yourself. For more details, visit the [official {syncdr} website](https://rossanatat.github.io/syncdr/), which I developed alongside the package itself. 
